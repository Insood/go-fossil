...
Application startup first creates an asset-free splash screen with its own Ark world and splash-specific input and render systems. The splash displays its title and start prompt until Space or gamepad A is pressed, then unloads its systems in reverse order and releases its world before `InitializeGame` creates the gameplay ECS and loads gameplay assets.
