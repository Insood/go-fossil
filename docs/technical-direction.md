...
`ArtifactCutoutDetectionSystem` scans damaged chunks every `artifactCutoutDetectionScanTicks`. It flood-fills remaining artifact ID regions, accepts regions smaller than `MaximumRegionSize`, scores the recovered pixels by artifact value and the rotated runtime artifact size, and records a fragment grade from 0 to 1 for the proportion of source artifact pixels recovered. Recovery at or above every source artifact's full pixel count produces a grade of 1. It creates in-memory fragments for regions at or above `artifactFragmentMinPixels`, queues one non-looping `pop.wav` playback request for each spawned fragment, clears the accepted overlay pixels, and softens the burn overlay.
...
Once started, pickup adds an ease-in movement targeting the live drone entity for 0.65 seconds while the pickup system tilts the plane toward the main camera and shrinks it. When homing completes, the fragment becomes collected, appears in the inventory UI with its score and color-coded grade (`[SUPER]` gold, `[A]` green, `[B]` yellow, `[C]` orange, or `[F]` red), and satisfies fragment-based tutorial progression; pickup does not change `Game.TotalScore`. Fragment plane models own their generated meshes; `ArtifactManager` continues to own the shared fragment textures and calculates carried weight. Drop-off frees capacity when a fragment is launched and marked dropped off. Its plane receives a linear movement with duration calculated from distance and the configured drop-off speed, and its score is added to `Game.TotalScore` only when the plane reaches the charging pad.
...
When a fragment reaches the charging pad, the drop-off system awards its score, queues battery recharge, and creates a `SoundPlaybackRequest` for the non-looping `score.wav` stream. `SoundSystem` consumes these ECS requests on its next update and retains additional requests for the same stream until each prior playback finishes.

## Entity Model

The current runtime entities are:

- one player drone with position, velocity, renderable model, drone and player-control tags, hover motion, laser, fire-control, and battery components
...
Application startup first creates a temporary splash scene with its own Ark world. It generates chunk `(0,0)`, then its four cardinal neighbors, then its four diagonal neighbors so available edges and corners remain continuous. A simulated drone starts over the center chunk, chooses a random full-speed X/Z heading once per second, moves through the normal physics system, stays clamped to loaded terrain, and retains its hover motion. Its additional splash movement region is a square with a 6-world-unit X/Z radius, centered 2 world units toward positive X and Z from the center chunk's geometric center. Player input does not control it, and the normal orthographic camera remains fixed on the center chunk.

`SplashScreenDroneFireTargetSystem` maintains an X/Z target offset that random-walks each update and remains clamped to `[-0.5, 0.5]` on both axes. It alternates between firing and cooldown states, choosing each state's duration independently from 0.2 to 0.6 seconds. While firing, it adds the offset to the drone's X/Z position, samples terrain height there, and writes one world target for the generic laser pipeline. The splash drone has a 1,000,000-charge battery and runs `LaserSystem` plus `ParticleSystem`, so its beam renders, burns terrain, and produces bounded-lifetime particles. The splash does not run laser sound, artifact cutout detection, or fragment systems, and it omits the drone viewport and gameplay UI.

`main` owns the disk-loaded `AssetManager` and passes it to both scene initializers. Pressing Space or gamepad A unloads the splash systems, all generated chunk meshes and textures, artifact resources, shadow framebuffer, managers, entities, and world before creating the gameplay ECS and authored default chunk with the same shared assets. After game over, the same inputs unload gameplay and return to a newly initialized splash; starting again creates a fresh gameplay world and score. `TutorialSystem` reports completion only after the player passes every step, and `main` retains that completion for the rest of the process. An incomplete tutorial is registered again on the next run; a completed tutorial is omitted. The existing gamepad quit input still exits instead of returning to the splash. Scene teardown never unloads the shared manager; `main` unloads it exactly once when the application exits.

- Input: gamepad quit and game-over menu-return handling in `InputSystem`; movement in `DroneInputSystem`; player aim/firing state in `PlayerDroneFireControlSystem`. Movement, aim, and fire-target systems require `PlayerControlled`, while the automated splash drone does not carry that marker. `PlayerFireInput` stores current and previous normalized drone viewport cursors plus firing state. Debug overlays release player fire control so the OS cursor can interact with raygui controls.
...
- Game over: `GameOverDetectionSystem` reacts once to a player-controlled drone at zero charge. It zeros horizontal velocity, applies a small downward velocity, removes `PlayerControlled` and `HoverMotion`, and creates a standalone `GameOver` marker. The state remains terminal even if an in-flight drop-off later supplies battery recharge.
- Motion: `PhysicsSystem` applies velocity; `DroneHeightSystem` snaps hovering drones to terrain height plus hover offset; `TerrainCollisionDetectionSystem` snaps velocity-driven entities that fall below sampled terrain back to terrain height and zeros their Y velocity. The disabled drone therefore sinks to terrain and settles there.
...
- Tutorial: `TutorialSystem` is mandatory on each gameplay run until the player completes every tutorial step once during the application process. It starts at step 1, advances to step 2 once the drone moves away from its starting X/Z position, spawns red shader-styled tutorial cones over artifact centers for step 2, advances to step 3 once the drone moves within 0.5 X/Z units of a cone, removes tutorial marker entities, advances to step 4 once the drone viewport cursor moves at least 25% of the drone viewport, advances to step 5 once any laser is active, advances to step 6 once the artifact manager has at least one collected fragment, queries the nearest `ChargingPad` entity to place the return-home cone, advances to step 7 once the drone is within `tutorialArtifactMarkerProximity` on X/Z of the pad, displays "Collect more!" for five seconds, and then reports completion. Tutorial updates and prompts pause while `GameOver` exists.
- Rendering: `RenderSystem3D` owns shadow, scene, drone viewport, laser rendering, and depth export.
- UI/debug: `UserInterfaceSystem`, `TutorialSystem`, `DebugRender3DSystem`, and `DebugRenderSystem2D` draw score, aligned labelled cargo and battery bars, undeposited fragment thumbnails, viewport, reticle, the active tutorial prompt, artifact labels, debug guides, shadow tuning controls, and the slope shade tuning control. The gold cargo bar uses collected, non-dropped fragment weight and excludes homing fragments. The battery bar is lime at 20% charge or higher and red below 20%. While `GameOver` exists, the gameplay HUD is replaced by centered `GAME OVER`, final-score text, and instructions to press Space or gamepad A to return to the title screen over the rendered 3D scene.
