...
Application startup first creates a temporary splash scene with its own Ark world. It generates chunk `(0,0)`, then its four cardinal neighbors, then its four diagonal neighbors so available edges and corners remain continuous. A simulated drone starts over the center chunk, chooses a random full-speed X/Z heading once per second, moves through the normal physics system, stays clamped to loaded terrain, and retains its hover motion. Its additional splash movement region is a square with a 6-world-unit X/Z radius, centered 2 world units toward positive X and Z from the center chunk's geometric center. Player input does not control it, and the normal orthographic camera remains fixed on the center chunk. The splash runs only simulated drone control, physics, drone height, light, main-scene rendering, start input, and text rendering; it omits the drone viewport and gameplay UI.
