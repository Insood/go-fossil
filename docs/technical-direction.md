...
Application startup first creates a temporary splash scene with its own Ark world. It generates chunk `(0,0)`, then its four cardinal neighbors, then its four diagonal neighbors so available edges and corners remain continuous. A non-player-controlled drone hovers over the center chunk while the normal orthographic camera remains fixed on that chunk. The splash runs only drone height, light, main-scene rendering, start input, and text rendering; it omits the drone viewport and gameplay UI.
