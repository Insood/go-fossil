...
Once started, pickup adds an ease-in movement targeting the live drone entity for 0.65 seconds while the pickup system tilts the plane toward the main camera and shrinks it. The fragment becomes collected, appears in the inventory UI with its score and color-coded grade (`[SUPER]` gold, `[A]` green, `[B]` yellow, `[C]` orange, or `[F]` red), and satisfies fragment-based tutorial progression only after the movement component is removed; pickup does not change `Game.TotalScore`. Fragment plane models own their generated meshes; `ArtifactManager` continues to own the shared fragment textures and calculates carried weight. Drop-off frees capacity when a fragment is launched and marked dropped off. Its plane receives a linear movement with duration calculated from distance and the configured drop-off speed, and its score is added to `Game.TotalScore` only when the plane reaches the charging pad.
